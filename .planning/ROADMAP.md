...
**Plans**: 4 plans
- [ ] 31-01-PLAN.md — FeishuSyncLog 模型 + SyncLogRead schema + Alembic 两阶段迁移（sync_type + mapping_failed_count）
- [ ] 31-02-PLAN.md — FeishuService 重构：_with_sync_log helper + _SyncCounters dataclass + 五方法走 helper + per-sync_type 锁
- [ ] 31-03-PLAN.md — API + Celery task：GET /sync-logs 过滤+分页 + CSV 下载端点 + trigger_sync 409 sync_type 语义 + performance_grades alias
- [ ] 31-04-PLAN.md — 前端：SyncLogsPage + 6 个子组件（Tab / 五色 badge / 4 色 status / 抽屉 / 空态 / 行）+ 菜单 + 路由 + UI-SPEC 6 维度 UAT
...
| 31. 飞书同步可观测性 | v1.4 | 0/4 | Not started | — |
