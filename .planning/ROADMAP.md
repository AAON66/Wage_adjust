...
**Execution Order:**
Phases execute in numeric order with data-integrity-first sequencing: 30 → 31 → 32 → 32.1 (INSERTED, 紧急 MVP 切片) → 33 → 34 → 35 → 36 → 37.
...
- **Phase 32.1 (员工端资格 MVP) 紧急插入** — 先于 Phase 33-35 交付 ESELF-01/02/04/05；不依赖 33/34（绩效档次推迟到 Phase 35 收尾）
- **Phase 33 (TierEngine 纯引擎) 先于 34 (Service + API)** — 延续 `EligibilityEngine` 分层模式，Service 消费 Engine
- **Phase 35 (员工端) 依赖 30 + 34 + 32.1** — 工号稳定 + 档次 API 就绪 + 资格 panel 已建好后，仅追加绩效档次徽章
...
**Plans**: 4 plans
- [x] 30-01-PLAN.md — FeishuSyncLog 模型扩展 + Alembic 迁移（leading_zero_fallback_count 字段）
- [x] 30-02-PLAN.md — ImportService 改造：Excel 模板文本格式 + 读入端 sanity check
- [x] 30-03-PLAN.md — FeishuService 改造：_map_fields 去 int 误用 + validate_field_mapping + 容忍匹配计数器
- [x] 30-04-PLAN.md — API + 前端：422 错误映射 + FeishuConfig 错误展示 + SyncStatusCard 黄色提示
...
**Plans**: 4 plans
- [x] 31-01-PLAN.md — FeishuSyncLog 模型 + SyncLogRead schema + Alembic 两阶段迁移（sync_type + mapping_failed_count）
- [x] 31-02-PLAN.md — FeishuService 重构：_with_sync_log helper + _SyncCounters dataclass + 五方法走 helper + per-sync_type 锁
- [x] 31-03-PLAN.md — API + Celery task：GET /sync-logs 过滤+分页 + CSV 下载端点 + trigger_sync 409 sync_type 语义 + performance_grades alias
- [x] 31-04-PLAN.md — 前端：SyncLogsPage + 6 个子组件（Tab / 五色 badge / 4 色 status / 抽屉 / 空态 / 行）+ 菜单 + 路由 + UI-SPEC 6 维度 UAT
...
**Plans**: 6 plans
- [x] 32-01-PLAN.md — DB schema + Wave 0 测试桩（ImportJob overwrite_mode/actor_id/updated_at + SalaryAdjustmentRecord UniqueConstraint + xlsx fixture builder）
- [x] 32-02-PLAN.md — ImportService 扩 6 类（hire_info/non_statutory_leave）+ overwrite_mode 矩阵 + salary_adjustments 改 upsert（Pitfall 4）
- [x] 32-03-PLAN.md — Service 层两阶段提交（PreviewResponse schema + is_import_running 锁 + expire_stale 双时限 + build_preview/confirm_import + AuditLog 真实字段）
- [x] 32-04-PLAN.md — API 层（preview/confirm/cancel/active 4 个端点 + 文件安全校验 + 旧端点 deprecated + 并发场景 e2e）
- [x] 32-05-PLAN.md — 前端类型契约 + service 层 5 个新函数（含 blob 下载）+ 6 个独立 UI 子组件 + ImportPreviewPanel 整合壳
- [x] 32-06-PLAN.md — ExcelImportPanel 7 态状态机改造 + APScheduler 定时清理 + E2E 集成测试 + 浏览器 UAT 4 项 checkpoint

### Phase 32.1: 员工端调薪资格自助可见 (紧急上线) (INSERTED)

**Goal**: 员工登录 MyReview 后在页面顶部看到本人当前调薪资格（合格 / 不合格 / 待定 三态徽章 + 4 条规则全展开 + 数据更新时间戳），无参数路由 `/eligibility/me` 让横向越权天然不可达；不依赖活动调薪周期，紧急 MVP 切片，先于 Phase 33-35 交付员工自助价值
**Depends on**: Phase 32 (导入功能稳定后再让员工端实时可见，避免显示陈旧/缺失数据)
**Requirements**: ESELF-01, ESELF-02, ESELF-04, ESELF-05 (ESELF-03 绩效档次推迟到 Phase 35)
**Success Criteria** (what must be TRUE):
  1. 员工登录 `/my-review` 页面后，在「当前角色」之后、「材料与提交概览」之前看到独立「本人调薪资格」section，不依赖 cycle 选择即可显示
  2. 顶部状态徽章 4 态正确呈现：eligible 绿底 ✓ / ineligible 红底 ✗ + 未通过项数 / pending 黄底 ⏳ + 数据缺失项数 / 未绑定灰底带跳「账号设置」链接
  3. 4 条规则（TENURE / ADJUSTMENT_INTERVAL / PERFORMANCE / LEAVE）全部摆开，按 status 染色（4 色左条 + 4 色 status badge），detail 文本不含 YYYY-MM-DD 格式日期或薪资数字
...
**Depends on**: Phase 30 (工号稳定) + Phase 34 (档次 API 就绪) + Phase 32.1 (资格 panel 已建好，本期仅追加档次徽章)
**Requirements**: ESELF-03 (ESELF-01/02/04/05 已由 Phase 32.1 交付)
**Success Criteria** (what must be TRUE):
  1. 员工登录后在 `MyReview` 页面已建好的「本人调薪资格」panel 下方追加绩效档次徽章（1/2/3 档），样本不足时显示「本年度全公司绩效样本不足，暂不分档」
...
**Execution Order:**
Phases execute in numeric order: 30 → 31 → 32 → 32.1 → 33 → 34 → 35 → 36 → 37.
