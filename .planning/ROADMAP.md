...
**Plans**: 6 plans
- [ ] 32-01-PLAN.md — DB schema + Wave 0 测试桩（ImportJob overwrite_mode/actor_id/updated_at + SalaryAdjustmentRecord UniqueConstraint + xlsx fixture builder）
- [ ] 32-02-PLAN.md — ImportService 扩 6 类（hire_info/non_statutory_leave）+ overwrite_mode 矩阵 + salary_adjustments 改 upsert（Pitfall 4）
- [ ] 32-03-PLAN.md — Service 层两阶段提交（PreviewResponse schema + is_import_running 锁 + expire_stale 双时限 + build_preview/confirm_import + AuditLog 真实字段）
- [ ] 32-04-PLAN.md — API 层（preview/confirm/cancel/active 4 个端点 + 文件安全校验 + 旧端点 deprecated + 并发场景 e2e）
- [ ] 32-05-PLAN.md — 前端类型契约 + service 层 5 个新函数（含 blob 下载）+ 6 个独立 UI 子组件 + ImportPreviewPanel 整合壳
- [ ] 32-06-PLAN.md — ExcelImportPanel 7 态状态机改造 + APScheduler 定时清理 + E2E 集成测试 + 浏览器 UAT 4 项 checkpoint
...
| 32. 调薪资格导入功能补齐 | v1.4 | 0/6 | Planning complete | — |
