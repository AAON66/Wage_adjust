...
- [ ] **ELIG-01**: 系统自动检查员工是否入职满 6 个月（基于 hire_date）
- [ ] **ELIG-02**: 系统自动检查距上次调薪（含转正调薪、专项调薪）是否满 6 个月（系统记录 + 导入历史）
- [ ] **ELIG-03**: 系统自动检查员工年度绩效是否为 C 级及以下（绩效数据支持 Excel 批量导入、飞书多维表格同步、管理端手动录入三种来源）
- [ ] **ELIG-04**: 系统自动检查员工年度非法定假期累计是否超过 30 天（排除产假等法定假期，请假数据支持 Excel 导入和飞书多维表格同步）
- [ ] **ELIG-05**: 资格校验结果仅在 HR/主管/管理端显示，员工端不可见
- [ ] **ELIG-06**: HR 可批量查看某部门/全公司的员工调薪资格状态
- [ ] **ELIG-07**: 不符合资格但有特殊情况的员工，部门可提交特殊申请（经 HR 和管理层审批）
- [ ] **ELIG-08**: 缺失数据源时（如绩效未导入），资格状态显示"数据缺失"而非直接判定不合格
