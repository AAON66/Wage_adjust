...
**执行计划：** `36-04`
**已完成自动工作：** `EvaluationDetail.tsx` 已挂载历史绩效面板，`tsc --noEmit` 已通过，浏览器自动化已覆盖 admin 正向与 employee 反向场景
...
- 已将 `PerformanceHistoryPanel` 挂载到 `activeModuleContent` 之后，避免只在调薪模块可见
...
| # | 角色 | 路径 | 预期 | 实际 | 结果 |
|---|------|------|------|------|------|
| 1 | admin | `/evaluations/{empA_eval}` | 显示历史绩效记录、按 year DESC、4 列齐全；同页共存断言通过 | Chrome 自动化实测：展示 2 条记录（2027、2026），4 列齐全，页面可见“历史绩效 / 年度绩效记录” | 通过（自动化） |
...
| 5 | empX | `/evaluations/{empA_eval}` | 若可进入页面，也不渲染 `PerformanceHistoryPanel` | Chrome 自动化实测：以 `test2@test.com` 打开同一 evaluation URL，可进入详情页但未出现“历史绩效 / 年度绩效记录” | 通过（自动化） |
...
| 8 | empX | `/my-review` | 不出现历史绩效面板，只保留员工端既有内容 | Chrome 自动化实测：`/my-review` 仅显示员工端既有资格、绩效档次、材料与证据内容，未出现“历史绩效 / 年度绩效记录” | 通过（自动化） |
...
- Chrome 自动化（frontend `127.0.0.1:5175`）：
  - `admin@company.com` 打开 `EvaluationDetail`，确认“历史绩效 / 年度绩效记录”面板可见，当前样本共 `2` 条记录，按 `2027 -> 2026` 倒序，4 列齐全
  - `test2@test.com` 打开同一 `EvaluationDetail` URL，页面未渲染历史绩效面板
  - `test2@test.com` 打开 `/my-review`，页面未渲染历史绩效面板
...
- 自动化期间发现 `PerformanceHistoryPanel` 最初仅挂在默认分支（实测等价于调薪模块），导致详情页概览首屏缺失；现已调整为挂在 `activeModuleContent` 之后并经 Chrome 复测通过
- 当前任务代码提交：`dd0414a` `feat(36-04): mount performance history in evaluation detail`

## Screenshots

- 待人工补充：hrbp / managerA / managerB / 空态 / Network 截图
...
- hrbp / managerA / managerB / 空态 / 浏览器 Network 场景尚未自动化覆盖，因此本文件当前不能视为最终验收结果
